...
class ApiException implements Exception {
  ...
  final int? businessCode; // 业务状态码 (如 code 或 resp_code)
  ...
  static String _extractServerMessage(Response? response) {
    if (response?.data != null) {
      ...
      if (data is Map) {
        ...
        // 尝试提取旧文档的 resp_msg 字段
        if (data['resp_msg'] != null &&
            data['resp_msg'].toString().isNotEmpty) {
        ...
        try {
          ...
          if (parsed['msg'] != null && parsed['msg'].toString().isNotEmpty) {
            return parsed['msg'].toString();
          }
          if (parsed['resp_msg'] != null &&
              parsed['resp_msg'].toString().isNotEmpty) {
  ...
  factory ApiException.fromDioException(DioException error) {
    switch (error.type) {
      ...
      case DioExceptionType.badResponse:
        ...
        // 提取业务错误码 (支持 code 或 resp_code)
        int? bCode;
        if (error.response?.data is Map) {
          bCode =
              error.response?.data['code'] ?? error.response?.data['resp_code'];
        }

        // 🌟 核心改进点：优先提取后端返回的 msg 或 resp_msg 错误细节
        ...
        // 如果后端没有返回任何包含 msg/resp_msg 的内容，再退回到本地通用的 HTTP 提示
        switch (statusCode) {
          case 400:
            return ApiException(
              statusCode: statusCode,
              businessCode: bCode,
              message: "语法错误，服务器无法理解请求",
            );
          case 401:
            return ApiException(
              statusCode: statusCode,
      ...
      case DioExceptionType.transformTimeout:
        return ApiException(message: "数据转换超时");
    }
  }
